...
const ProfileEditModal = ({ isOpen, onClose, profile, onUpdate }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 md:p-4 bg-background/90 backdrop-blur-md animate-in fade-in duration-300 overflow-y-auto">
            ...
            <div className="relative w-full max-w-xl bg-surface-container-low border border-primary/20 p-6 md:p-10 shadow-[0_0_80px_rgba(252,238,10,0.05)] overflow-hidden group my-auto">
                {/* Tech Corners - Refined */}
                <div className="absolute top-0 left-0 w-8 md:w-12 h-8 md:h-12 border-t-4 border-l-4 border-primary"></div>
                <div className="absolute bottom-0 right-0 w-8 md:w-12 h-8 md:h-12 border-b-4 border-r-4 border-primary"></div>
                <div className="absolute top-0 right-0 w-4 md:w-6 h-4 md:h-6 border-t-2 border-r-2 border-secondary/40"></div>
                <div className="absolute bottom-0 left-0 w-4 md:w-6 h-4 md:h-6 border-b-2 border-l-2 border-secondary/40"></div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 md:mb-10 border-b border-primary/10 pb-6 relative">
                    <div className="flex flex-col">
                        <span className="text-[8px] md:text-[10px] text-primary/40 font-code-md tracking-[0.4em] mb-1">CONFIGURAÇÕES_DE_CONTA</span>
                        <h2 className="text-2xl md:text-4xl font-black text-primary uppercase tracking-tighter italic leading-none drop-shadow-[0_0_8px_rgba(252,238,10,0.4)]">
                            EDITAR_PERFIL
                        </h2>
                    </div>
                    <button 
                    ...
                    <div className="absolute -bottom-[1px] left-0 w-16 md:w-24 h-[2px] bg-secondary shadow-[0_0_10px_rgba(0,243,255,0.8)]"></div>
                </div>

                <form onSubmit={handleSave} className="space-y-4 md:space-y-5 relative">
                    ...
                    <fieldset className="flex flex-col gap-2 mb-2 group/field">
                        ...
                        <div className="relative">
                            <textarea 
                                className="w-full bg-background/50 border-b-2 border-primary/10 focus:border-secondary text-on-surface font-code-md text-sm px-4 py-4 outline-none transition-all placeholder:text-on-surface-variant/20 min-h-[100px] md:min-h-[120px] resize-none scrollbar-hide"
                                value={bio}
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:gap-4 pt-4 md:pt-8">
                        <button
                            type="button"
                            onClick={onClose}
                            className="w-full md:flex-1 py-3 md:py-4 border-2 border-tertiary/30 text-tertiary font-black uppercase tracking-[0.1em] text-xs hover:bg-tertiary/10 hover:border-tertiary transition-all cursor-pointer relative group/cancel overflow-hidden"
                        >
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full md:flex-1 relative overflow-hidden bg-primary text-background group-hover/btn:text-secondary font-black uppercase tracking-[0.2em] text-xs py-3 md:py-4 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 cursor-pointer group/btn shadow-[0_0_20px_rgba(252,238,10,0.15)]"
                        >
